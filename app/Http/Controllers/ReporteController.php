                    ...
                    if($modulo->nombre == 'TASAS'){
                        ...
                        foreach($modulo->conceptos as $concepto){
                            ...
                            if($concepto->nompre == 'TASAS INTERNACIONALES MODULO'){  
                                ...
                                $montos[$meses[$diaMes->month]][$modulo->nombre][$concepto->nompre] = \App\Tasaop::join('tasaopdetalles', 'tasaops.id', '=', 'tasaopdetalles.tasaop_id')
        ...
        for(;$primerDiaMes<=$ultimoDiaMes; $primerDiaMes->addDay()){
            ...
            foreach ($modulos as $modulo) {
                ...
                if($modulo->nombre == 'TASAS'){
                        ...
                        $montos[$primerDiaMes->format('d/m/Y')][$modulo->nombre]["total"]    += $tasas;
                        ...
                        foreach($modulo->conceptos as $concepto){
                            ...
                            if($concepto->nompre == 'TASAS INTERNACIONALES MODULO'){   
                                ...
                                $montos[$primerDiaMes->format('d/m/Y')][$modulo->nombre][$concepto->nompre] = \App\Tasaop::join('tasaopdetalles', 'tasaops.id', '=', 'tasaopdetalles.tasaop_id')
                                                            ->whereIn('tasaops.id', $tasasIntMod)
                                                            ->whereIn('tasaopdetalles.serie', $tasasInternacionales)
                                                            ->sum('tasaopdetalles.total');
        ...
        if($nFactura != 0){
            ...
            foreach ($recibos as $index => $recibo) {
                if($recibo->montodepositado != $recibo->montofacturas){
                    $monto = Ajuste::where('cliente_id', $recibo->cliente_id)->orderBy('id', 'DESC')->first();
                    if(!isset($ajustes[$recibo->id]))
                        $ajustes[$recibo->id] = 0;
                    if($monto->cobro_id != $recibo->id)
                        $ajustes[$recibo->id] = ($monto->monto < 0)?(($recibo->montodepositado-$recibo->montofacturas) + $monto->monto):($recibo->montodepositado-$recibo->montofacturas);
                    else
                        $ajustes[$recibo->id] = ($monto->monto < 0)?($recibo->montodepositado + $monto->monto):($recibo->montodepositado-$recibo->montofacturas);
                }
            }            
        }
    ...
    public function getReporteRelacionMetaRecaudacionMensual(Request $request){
       ...
       for($i=1;$i<=12; $i++){
            ...
            foreach ($cobrosSNV as $cobroSNV) {
              ...
              foreach ($tasasPZO as $tasaPZO) {
                $montosMeses[$meses[$diaMes->month]]["recaudadoPZO"]+=$tasaPZO->monto;
            }

            foreach ($tasasCBL as $tasaCBL) {
                $montosMeses[$meses[$diaMes->month]]["recaudadoCBL"]+=$tasaCBL->monto;
            }

            foreach ($tasasSNV as $tasaSNV) {
                $montosMeses[$meses[$diaMes->month]]["recaudadoSNV"]+=$tasaSNV->monto;
            }
    ...
    public function getReporteResumenFacturasAeronauticasCredito (Request $request){
        ...
        foreach ($facturas as $factura) {
            foreach ($factura->cobros as $recibo){
                ...
                $dosaFactura[$recibo->id]["totalDepositado"] =$recibo->montodepositado;
                $dosaFactura[$recibo->id]["reciboCaja"]      =$recibo->nRecibo;
                $dosaFactura[$recibo->id]["fecha"]           =$factura->fecha;
                foreach ($recibo->pagos as $pago){
        ...
        return view('reportes.reporteRelacionFacturasAeronauticasCreditoResumen', compact('aeropuertoNombre', 'diaDesde', 'mesDesde', 'annoDesde', 'diaHasta', 'mesHasta', 'annoHasta', 'aeropuerto', 'cliente', 'facturas', 'dosaFactura', 'clientes'));
