...
<ol class="breadcrumb">
	...
	<li><a class="active">Facturación Manual</a></li>
</ol>
	...
	<div class="col-md-12">
		<div class="box box-primary">
			...
			<div class="box-body text-right"  id="container">
				{!! Form::open(["url" => action('FacturaController@index',["id"=>'TODOS']), "method" => "GET", "class"=>"form-inline"]) !!}
				...
				<div class="form-group">
					...
					<div class="input-group">
						<div class="input-group-btn">
							<button style="max-height:37px" type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="operator-text">{{array_get( $input, 'totalOperator')}}</span></button>
							<ul class="dropdown-menu operator-list">
								<li><a href="#">>=</a></li>
								<li><a href="#"><=</a></li>
								<li><a href="#">=</a></li>
				...
				<a class="btn btn-default"  href="{{ URL::to('facturacion/'.'TODOS'.'/factura') }}" }}">Reset</a>
				{!! Form::close() !!}
			</div>
		</div>
	</div>
								...
								<div class="media">
									...
									<div class="pull-right media-right">
										<div class="btn-group-vertical  btn-group-xs" role="group" aria-label="...">
											<a class="btn btn-primary"  href="{{ URL::to('facturacion/'.'TODOS'.'/factura/create') }}">&nbsp;<span class="glyphicon glyphicon-plus"></span>&nbsp;</a>
										</div>
									</div>
							...
							<li class="list-group-item">
										...
										<tr>
											...
											<td>
												<div class='btn-group  btn-group-sm' role='group' aria-label='...'>
													...
													<a class='btn btn-warning' href='{{url('facturacion/TODOS/factura/'.$factura->id.'/edit')}}'><span class='glyphicon glyphicon-pencil' ></span></a>
													<button class='btn btn-danger eliminar-factura-btn' data-id="{{$factura->id}}"><span class='glyphicon glyphicon-remove'></span></button>
													<a target="_blank" class='btn btn-default' href='{{action('FacturaController@getPrint', ['modulo' => 0, $factura->id])}}'><span class='glyphicon glyphicon-print' ></span></a>
								...
								<div class="row">
									<div class="col-xs-12 text-center">
										{!! $facturas->appends(Input::except('page'))->render() !!}
									</div>
								</div>
...
<script>
	...
	$(document).ready(function(){
		$('#show-modal').on('show.bs.modal', function (e) {
			...
			.load("{{url("facturacion/".urlencode ('TODOS')."/factura")}}/"+id, function(){
		...
		$('.eliminar-factura-btn').click(function(){
			...
			var id=$(this).data("id");
			var url="{{url('facturacion/'.'TODOS'.'/factura')}}/"+id;
